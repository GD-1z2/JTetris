...
public class Renderer
{
	...
	{
		quadData(x, y, width, height, color, 0, 0, 1, 1);
	}
	...
	public static void renderTile(int x, int y, float[] color) {
		glBegin(GL_QUADS);
		quadData(x*Main.TILE_SIZE, y*Main.TILE_SIZE, Main.TILE_SIZE, Main.TILE_SIZE, new float[] {0,0,0,1});
	   	glEnd();
		
		glBegin(GL_QUADS);
		quadData(x*Main.TILE_SIZE+(int)(Main.TILE_SIZE*.04), y*Main.TILE_SIZE+(int)(Main.TILE_SIZE*.04), (int)(Main.TILE_SIZE*.91), (int)(Main.TILE_SIZE*.91), new float[] {color[0]*2f,color[1]*2f,color[2]*2f,1});
		glEnd();
